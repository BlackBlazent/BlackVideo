...
interface VideoDualRecorderControllerProps {
  initialFacing: 'user' | 'environment';
  isVisible: boolean;
  onClose: () => void;
}

const VideoDualRecorderController: React.FC<VideoDualRecorderControllerProps> = ({ 
  initialFacing, 
  isVisible, 
  onClose 
}) => {
  ...
  const [size, setSize] = useState({ width: 360, height: 'auto' as number | 'auto' });
  const [resizeStart, setResizeStart] = useState({ x: 0, y: 0, width: 360, height: 450 });
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isResizing && controlsRef.current) {
        ...
        const newWidth = Math.max(320, Math.min(550, resizeStart.width + deltaX));
        const newHeight = Math.max(400, Math.min(650, resizeStart.height + deltaY));
  ...
  const initializeRecording = async () => {
    try {
      ...
      // Initialize both video and camera
  ...
  const startRecording = async () => {
    ...
    try {
      await recorderRef.current.startRecording('both');
      setIsRecording(true);
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      if (recordedBlob) {
        ...
        await saveRecording(recordedBlob, `dual-${cameraFacing}-${Date.now()}`);
      }
      // Close after saving
      onClose();
    } catch (err) {
      ...
      <div className="recorder-controls-panel">
        ...
        <div className="preview-section">
          <div className="video-status">
            ...
            <div className="status-text">
              ...
              <small>{cameraFacing === 'user' ? 'Front' : 'Back'} Camera + Popup Display</small>
            </div>
          </div>

          {/* Dual recording visual indicator */}
          <div className="dual-recording-indicator">
            <div className="dual-source">
              <Monitor size={16} />
              <span>Display</span>
            </div>
            <div className="dual-plus">+</div>
            <div className="dual-source">
              <Layers size={16} />
        ...
        <div className="recording-controls">
          <div className="recorder-timer-display">
            ...
            {/* Duration bar at max (jumps to end) */}
          ...
          <div className="control-buttons">
            {!isRecording ? (
              <button className="start-btn" onClick={startRecording} disabled={!!error}>
                <Layers size={14} /> Start Dual Recording
              </button>
            ) : (
        ...
        <div className="recording-info">
          <div className="info-row">
            ...
            <span className="info-value">Dual (PiP)</span>
          </div>
          <div className="info-row">
...
export default VideoDualRecorderController;
