...
import {
  extensionContextMenuManager,
  ExtensionSettings,
} from '../playgroundExtensionPlaybackContextMenu';
...
export const PlaygroundExtensionPlaybackContextMenu: React.FC<ContextMenuProps> = ({ targetElementId }) => {
  const [isVisible, setIsVisible] = useState(false);
  const [position, setPosition] = useState({ x: 0, y: 0 });
  const [settings, setSettings] = useState<ExtensionSettings>(extensionContextMenuManager.getSettings());
  ...
  useEffect(() => {
    const unsubscribe = extensionContextMenuManager.subscribe((newSettings) => { setSettings(newSettings); });
    return () => unsubscribe();
  }, []);
  ...
  const menuOptions = extensionContextMenuManager.getContextMenuOptions();
...
export default PlaygroundExtensionPlaybackContextMenu;
