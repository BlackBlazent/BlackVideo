...
import {
  customPlaybackContextMenuManager,
  CustomPlaybackVisibility,
} from '../playgroundCustomPlaybackContextMenu';
...
export const PlaygroundCustomPlaybackContextMenu: React.FC<ContextMenuProps> = ({
  targetElementId,
}) => {
  const [isVisible, setIsVisible] = useState(false);
  const [position, setPosition] = useState({ x: 0, y: 0 });
  const [visibility, setVisibility] = useState<CustomPlaybackVisibility>(
    customPlaybackContextMenuManager.getVisibility()
  );
  ...
  useEffect(() => {
    const unsubscribe = customPlaybackContextMenuManager.subscribe((newVisibility) => {
  ...
  const menuOptions = customPlaybackContextMenuManager.getContextMenuOptions();

  return (
    <>
      <div ref={menuRef} className="playground-context-menu" style={{ left: `${position.x}px`, top: `${position.y}px` }}>
...
export default PlaygroundCustomPlaybackContextMenu;
