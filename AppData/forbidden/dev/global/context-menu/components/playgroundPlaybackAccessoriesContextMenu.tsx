...
import {
  ...
  AccessoriesSettings,
} from '../playgroundPlaybackAccessoriesContextMenu';
...
export const PlaygroundPlaybackAccessoriesContextMenu: React.FC<ContextMenuProps> = ({ targetElementId }) => {
  const [isVisible, setIsVisible] = useState(false);
  const [position, setPosition] = useState({ x: 0, y: 0 });
  const [settings, setSettings] = useState<AccessoriesSettings>(accessoriesContextMenuManager.getSettings());
  ...
  useEffect(() => {
    const unsubscribe = accessoriesContextMenuManager.subscribe((newSettings) => { setSettings(newSettings); });
    return () => unsubscribe();
  }, []);
  ...
  const menuOptions = accessoriesContextMenuManager.getContextMenuOptions();
...
export default PlaygroundPlaybackAccessoriesContextMenu;
